...
class ChatViewController: UIViewController {
    ...
    @IBAction func sendMessage() {
        ...
        if mqttVersion == "3.1.1" {
            mqtt!.publish(firstTopic + "/chat/room/animals/client/" + animal!, withString: message!, qos: .qos1)
        }else if mqttVersion == "5.0" {
            mqtt5!.publish(firstTopic + "/chat/room/animals/client/" + animal!, withString: message!, qos: .qos1, DUP: true, retained: false, properties: publishProperties)
        }
    ...
    @objc func receivedMessage(notification: NSNotification) {
        ...
        let sender = topic.replacingOccurrences(of: firstTopic + "/chat/room/animals/client/", with: "")
